...
# RO:WHAT — BUILD_PLAN4 Phase 7 actual capped ROC-to-ROX private testnet receipt checker.
# RO:WHY — Validates redacted send/readback receipts without calling RPC, signing, submitting, minting, burning, or mutating ROC.
# RO:INTERACTS — docs/pilot/ACTUAL_ROC_TO_ROX_PRIVATE_TESTNET_RUN.md, .gitignore, local ignored receipts.
# RO:INVARIANTS — devnet/testnet only; shadow ROC burn only; explicit approval; tiny caps; test-only ROX; readback required.
# RO:SECURITY — local file checks only; no wallet/key load, RPC, signing, submission, mint, burn, settlement, or ROC mutation.
...
usage() {
  cat <<'USAGE'
usage:
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --check-docs [repo-root]
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --preflight [repo-root] [devnet|testnet]
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --template-send-sent [devnet|testnet]
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --template-send-blocked [devnet|testnet]
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --template-readback [devnet|testnet]
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --check-send-receipt <receipt-json>
  bash scripts/check_actual_roc_to_rox_private_testnet_run.sh --check-readback-receipt <receipt-json>
USAGE
}
...
check_docs() {
  ...
  local doc="$root/docs/pilot/ACTUAL_ROC_TO_ROX_PRIVATE_TESTNET_RUN.md"
  local script="$root/scripts/check_actual_roc_to_rox_private_testnet_run.sh"
  local gitignore="$root/.gitignore"

  [ -f "$doc" ] || fail "missing docs/pilot/ACTUAL_ROC_TO_ROX_PRIVATE_TESTNET_RUN.md"
  [ -f "$script" ] || fail "missing scripts/check_actual_roc_to_rox_private_testnet_run.sh"
  ...
  do
    grep -Fq -- "$needle" "$doc" || fail "ROC-to-ROX runbook missing marker: $needle"
    ok "doc marker present: $needle"
  done

  for ignored in \
    "actual-roc-to-rox-capped-send-receipt.json" \
    "actual-roc-to-rox-capped-send-receipt.local.json" \
    "actual-roc-to-rox-capped-send-failed.local.json" \
    "actual-roc-to-rox-readback.local.json" \
    "actual-roc-to-rox-readback-failed.local.json" \
    "*.actual-roc-to-rox-capped-send.local.json" \
    "*.actual-roc-to-rox-capped-send-receipt.local.json" \
    "*.actual-roc-to-rox-capped-send-failed.local.json" \
    "*.actual-roc-to-rox-readback.local.json" \
    "*.actual-forward-flow.local.json"
  do
    grep -Fq "$ignored" "$gitignore" || fail ".gitignore missing ROC-to-ROX receipt ignore marker: $ignored"
    ok ".gitignore covers $ignored"
  done

  reject_sensitive_text "$doc" "ROC-to-ROX runbook"
  ...
  do
    if grep -Fq "$forbidden" "$doc"; then
      fail "ROC-to-ROX runbook contains forbidden claim marker: $forbidden"
    fi
    ok "doc excludes forbidden claim marker: $forbidden"
  done

  cat <<'SUMMARY'
ok: BUILD_PLAN4 Phase 7 ROC-to-ROX documentation checks passed
summary:
  - actual capped ROC-to-ROX private testnet runbook exists
  - local capped-send/readback receipt names are ignored
  - documentation preserves shadow-ROC-only, test-only-ROX, redacted, capped, non-mainnet boundaries
  - documentation separates capped send evidence from readback verification, real ROC mutation, production settlement, and finality
SUMMARY
}
...
print_send_template() {
  ...
  if [ "$outcome" != "sent" ]; then
    ...
    failure_line='  "failure_reason_redacted": "<redacted-safe-capped-send-blocker>",'
  fi
...
$failure_line
  ...
  "send_slot": "$slot",
  "test_only_rox_delta_minor": "$delta",
...
check_send_receipt() {
  ...
  reject_sensitive_text "$receipt" "ROC-to-ROX send receipt"

  require_json_string "$receipt" "schema" "rox-anchor.actual-roc-to-rox-capped-send.v1"
  require_json_string "$receipt" "phase" "BUILD_PLAN4 Phase 7"
  require_json_string "$receipt" "receipt_role" "actual_roc_to_rox_capped_send_receipt"
  require_json_string "$receipt" "direction" "roc_to_rox"
  ...
  for field in \
    program_id operation_id idempotency_key nonce shadow_roc_burn_intent_id \
    ...
    signer_path_redacted receipt_out_redacted transaction_signature send_slot test_only_rox_delta_minor
  do
  ...
  local amount max_amount max_operations retry_cap delta
  ...
  delta="$(json_string_value "$receipt" "test_only_rox_delta_minor")"
  ...
  require_integer_string "$delta" "test_only_rox_delta_minor"
  [ "$amount" -le "$max_amount" ] || fail "amount_minor cannot exceed max_amount_minor"
  ok "amount cap relation is valid"

  require_json_bool_true "$receipt" "shadow_roc_burn_only"
  ...
  require_json_bool_false_or_absent "$receipt" "real_roc_burn"
  ...
  if [ "$outcome" = "sent" ]; then
    ...
    [ "$delta" = "$amount" ] || fail "sent receipt test_only_rox_delta_minor must equal amount_minor"
    ok "sent receipt satisfies capped send gates"
  else
    ...
    [ "$delta" = "0" ] || fail "blocked/failed receipt must not claim test-only ROX delta"
    ok "blocked/failed receipt remains non-submitting evidence"
  fi

  cat <<'SUMMARY'
ok: BUILD_PLAN4 Phase 7 ROC-to-ROX send receipt checks passed
summary:
  - receipt is devnet/testnet only
  - receipt is ROC-to-ROX only
  - shadow ROC burn is explicitly not real ROC burn
  ...
  - receipt does not claim public launch, mainnet, production settlement, public ROX mint/burn, real ROC burn, real ROC mutation, or finality
SUMMARY
}
...
check_readback_receipt() {
  ...
  reject_sensitive_text "$receipt" "ROC-to-ROX readback receipt"

  require_json_string "$receipt" "schema" "rox-anchor.actual-roc-to-rox-readback.v1"
  require_json_string "$receipt" "phase" "BUILD_PLAN4 Phase 7"
  require_json_string "$receipt" "receipt_role" "actual_roc_to_rox_readback_receipt"
  require_json_string "$receipt" "direction" "roc_to_rox"
  require_json_string "$receipt" "program_name" "rox_anchor"
  ...
  for field in \
    ...
    program_account config_account test_only_mint test_only_token_account \
    expected_test_only_rox_delta_minor observed_test_only_rox_delta_minor rpc_evidence_redacted
  do
    require_json_string_present "$receipt" "$field"
  done
  ...
  expected="$(json_string_value "$receipt" "expected_test_only_rox_delta_minor")"
  observed="$(json_string_value "$receipt" "observed_test_only_rox_delta_minor")"

  require_tiny_positive_amount "$expected" "expected_test_only_rox_delta_minor" 1000
  require_tiny_positive_amount "$observed" "observed_test_only_rox_delta_minor" 1000
  [ "$expected" = "$observed" ] || fail "observed test-only ROX delta must match expected delta"
  ok "readback delta matches expected delta"
  ...
  cat <<'SUMMARY'
ok: BUILD_PLAN4 Phase 7 ROC-to-ROX readback receipt checks passed
summary:
  - readback receipt is devnet/testnet only
  - readback evidence is read-only RPC
  - redacted accounts, signature, receipt ID, and RPC evidence are enforced
  - observed test-only ROX delta matches expected delta
  - receipt does not claim submission, public launch, mainnet, production settlement, public ROX mint/burn, real ROC burn, real ROC mutation, or finality
SUMMARY
}
...
preflight() {
  ...
  if [ -d "$root/.git" ] && command -v git >/dev/null 2>&1; then
    if git -C "$root" ls-files | grep -Eq '(^|/)(actual-roc-to-rox.*\.json|.*actual-forward-.*\.json)$'; then
      fail "git tracked actual ROC-to-ROX receipt material found"
    fi
    ok "git tracked-file scan found no actual ROC-to-ROX receipt material"
  else
    ok "git tracked-file scan skipped because git metadata is unavailable"
  fi

  cat <<'SUMMARY'
ok: BUILD_PLAN4 Phase 7 ROC-to-ROX preflight passed
summary:
  - ROC-to-ROX documentation and ignore boundaries are present
  - Anchor build outputs exist
  - Phase 5 read-only evidence checker exists
  - Phase 6 simulation checker exists
  - no tracked ROC-to-ROX receipt material was found
  - this preflight did not call RPC, submit, sign, load a signer, mint, burn, settle, or mutate ROC
SUMMARY
}
